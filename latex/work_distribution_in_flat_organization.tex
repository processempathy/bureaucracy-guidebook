...
The concept of every member being able to do every task is the model 
...
The simplest non-trivial scenario is when not every member of an organization can do every task. There are two specializations (A, B) and one skill level: 
...
Task throughput is a metric for comparing different configurations of the model. Rather than count tasks per tick during steady-state, here we count number of tasks completed in 100 ticks. 
...
\caption{Tasks completed by each person in an organization with six people. Persons 3 and 5 both have maximum throughput of tasks since they are unicorns. Person 0 has no throughput and only delegates tasks. Everyone is busy (no one is idle), but the level of productivity (tasks completed per time) varies.}
...
Conclusions from this configuration:
\begin{itemize}
    \item The reward for good work is more work.
    \item Coordination means lower throughput (with social size 0)
\end{itemize}


\begin{figure}
\centering
\includegraphics{images/task_distribution_completed_versus_size_simCount25_skills2_levels1_taskduration1_people2_social0_ticks100.pdf}
\caption{Task throughput after 100 ticks versus organization size. 25 simulations were run for each organization size. 
Adding people does increase throughput, but the variance of throughput increases due to dependence on the the distribution of specializations and skill-levels of the participants. The instances where task completion is near zero occur when no members of the organization have sufficient skills to address tasks. 
...
Functions written in Python 3 used to generate data used for figures in this appendix.

The entity in this simulation is a person. 
