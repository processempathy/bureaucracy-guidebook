...
In the numerical model the random selection of someone else who has the necessary specialization and skill-level is insensitive to the size of their backlog. This is reflects my experience in  large organizations where task backlogs are not visible. As a consequence you merely add something to their backlog and hope they will get to it eventually.
...
\begin{itemize}
    ...
    \item No improvement of skill-level per person, or additional specialization.
\end{itemize}
...
A person with skill-level 3 in specialization A and skill-level 1 in specialization C would be labeled ``A3,C1''. A visualization of their capabilities would be the following table:
...
A generalist (here~``A1,B1,C1") is not an expert at anything, but is capable in multiple specializations. A unicorn (here a~``A3,B3,C3") can do everything well.
\begin{center}
Generalist:
$\left\{
\begin{tabular}{c|c|c|c|}
...
\href{https://en.wikipedia.org/wiki/Adhocracy}{adhocracy} advocates. The feasibility of gathering people with the relevant talents becomes challenging as the task complexity increases or increased throughput is needed. That is why organizations hire people who are not unicorns. 
...
\caption{Tasks completed by each person in an organization with six people. Rather than count tasks per tick during steady-state, here we count number of tasks completed in 100 ticks. Persons 3 and 5 both have maximum throughput of tasks since they are unicorns. Person 0 has no throughput and only delegates tasks. Everyone is busy (no one is idle), but the level of productivity (tasks completed per time) varies.}
...
The need for coordination can be blamed on the random delegation of task not being aligned to the specializations and skill-levels of the workers. 
...
Adding people does increase throughput, but the variance of throughput increases due to dependence on the the distribution of specializations and skill-levels of the participants. The instances where task completion is near zero occur when no members of the organization have sufficient skills to address tasks. That condition is less likely as the number of people increases.
...
Because the specialization of skills is too diverse for anyone person to master everything. This is easy to understand when the organization is society. It would be faster for you to 
 inspect your own meat for food safety, monitor the emissions of a coal-fired power plan, build your own semiconductor chips, manufacture your own airplane. However, you don't have all those skills and you want the results quicker than you can complete the tasks yourself. Therefore, the tasks are distributed among an organization of people with different skills. The cost of coordination is that the throughput is lower.
...
Because unicorns can do any task, one way to improve throughput is to invest in training people. Training in the context of this model have the purpose of  increased breadth (additional specializations) or increased depth (improved skill-level for a specialization). One reason there are not more unicorns in an organization is that not everyone wants to know and do everything. Knowing how to create a webpage, analyze data, manage a server, initiate a contract, interact with customers, gather requirements, and run a team can feel overwhelming. Another reason unicorns are rare is turn-over. When a person outgrows their assigned role,  they may look to other opportunities to feel more fulfilled. Retaining unicorns is challenging.

A separate approach to having too many tasks relative to the number of people in the workforce is to prioritize the work. Prioritization doesn't necessarily increase task throughput, but without prioritization, tasks get started and may not get completed. (The problem of ``start task but don't finish it" isn't part of the numerical model but does happen in real organizations.)

Prioritization requires sacrifice (what tasks are you not doing) and induces risk (there is consequence to not doing tasks) and spends your reputation (not doing tasks effects other people).

There are two conditions under which prioritization becomes important:
\begin{itemize}
    \item The number of tasks is less than or equal to the amount of staff attention available and skill. Then prioritization is merely ordering of the sequence of tasks.
    \item When the number of tasks exceeds staff and skill-level per person, an oversubscribed person won't get to every task. Saying no is important but harms your reputation.
\end{itemize}
...
Static prioritization that is exclusive. Only work on one thing to the exclusion of everything else.
    ...
    \item Prioritization proportionally allocated to task categories. For example, suppose you have three categories of tasks. You allocate 10\% of your time for the first category, 20\% of your time for the second, and your remaining time to  the third category. 
\end{itemize}

\clearpage
\section{Software implementation of Simulation Model}
