...
\newglossaryentry{attention-time}{
name={attention-time},
description={Attention can be at various levels, from distracted to intensely focused. The amount of time you spend can be quick or long. The concept of attention-time is the product of those two variables. Spending lots of time but with low-engagement is probably not equivalent to a short duration of focused attention -- the concept is not 
\index{Wikipedia!ergodic process@\string\href{https://en.wikipedia.org/wiki/Ergodic_process}{ergodic process}}
\href{https://en.wikipedia.org/wiki/Ergodic_process}{ergodic}}
}

\newglossaryentry{culture}{
name={culture},
plural={cultures},
description={Also known as norms. Expectations each person has regarding interactions with other people}
}
...
name={bikeshedding},
description={The recipient of a question or request focuses on unimportant details relative to the primary topic. See 
\index{Wikipedia!organizational chart@\string\href{https://en.wikipedia.org/wiki/Law_of_triviality}{Law of Triviality}}
Wikipedia entry for \href{https://en.wikipedia.org/wiki/Law_of_triviality}{Law of Triviality}}
}
...
name={participant},
description={A person who is expected to take action or make a contribution}
}
...
staffing, and skills necessary to address the complexity, latency, and scale of the challenge. In contrast to accidental and legacy bureaucracy}
}
...
description={The cost of work needed to change a \gls{process} caused by choosing an easy solution now instead of using a better approach that would take longer or cost more money. Bureaucratic debt is a trade-off of what work to do and risks to take. The trade-off can be intentional or can go unrecognized. More effort (work, time) could be spent building a better product, but subjects want solutions for access to the shared resource now}
}
...
\newglossaryentry{bureaucrat}{
    name={bureaucrat},
    plural={bureaucrats},
    description={The person responsible for enacting someone else's policy for access to 
...
is concerned with emotions like sadness, happiness, anger. See also \gls{intellectual empathy}}
}
...
you can predict another person's actions}
}

\newglossaryentry{decision archaeology}{
name={decision archaeology},
description={Identifying the evolutionary sequence of decisions that started with a specific issue and led to the current situation. 
...
\index{Wikipedia!Chesterton's fence@\string\href{https://en.wikipedia.org/wiki/G._K._Chesterton\%23Chesterton\%27s_fence}{Chesterton's fence}}
...
For example pay, promotion, titles, stability, and awards. In contrast to \gls{motive}}
}

\newglossaryentry{process empathy}{
name={process empathy},
description={Cultivating process empathy requires considering factors like
\begin{itemize}
\item Internal 
...
\begin{itemize}
\item A play put on by actors for an audience. The play is the shared resource; the norms of watching a play are the informal policies. This is not a bureaucracy because violations are addressed by the policymakers -- the actors. There are also policies set by the theater. Theater policies form a bureaucracy because ushers are the bureaucrats and the audience are subjects. 
...
\gls{shared resource}.
The resource can be either tangible, in the form of expertise, or information. \\
Managing access to shared resources does not require bureaucracy (e.g., if there is consensus, or through the use of violence) and bureaucracy can exist without a shared resource being managed (e.g., malicious  processes inflicted by (threat of) violence.) \\
\index{Wikipedia!wicked problem@\string\href{https://en.wikipedia.org/wiki/Wicked_problem}{wicked problem}}
    ...
    There are two subtopics that depend on the number of people involved:  
    \gls{core bureaucracy}
    and 
    \gls{decentralized bureaucracy}
}

\newglossaryentry{decentralized bureaucracy}{
    name={decentralized bureaucracy},
    text={decentralized bureaucracy},
    ...
    Arises when different bureaucrats work in opposition because they are referencing different policies. As a consequence, the \gls{subject} is burdened with resolution but doesn't have the authority to resolve the dissonance. Processes can appear nonsensical from the holistic perspective.\\
    Necessary when no one bureaucrat can carryout the relevant policies (due to complexity or size). \\Distributed knowledge and distributed decision-making are essential}
}

\newglossaryentry{core bureaucracy}{
    name={core bureaucracy},
    text={core bureaucracy},
    description={The separate roles of \gls{policymaker}, \gls{bureaucrat}, and \gls{subject}. A minimum of three separate people occupy the three roles. The relevance of three different people is the inability of the subject to directly negotiate with the policymaker. The separation of roles introduces frustration for the subject. \\
    Examples not meeting the definition of core bureaucracy due to the lack of separate bureaucrat:
    \begin{itemize}
        \item The manager of swimming pool that has no other staff.
        \item A ``no parking" sign.
    \end{itemize}
Each of the above two scenarios have a shared resource, policymaker, and subject}
}
...
are known to some 
\iftoggle{glossarysubstitutionworks}{\glspl{stakeholder}}{stakeholders} 
and are conveyed verbally to some of the other stakeholders. In contrast to \gls{visible bureaucracy}}
}
...
enforced by 
\iftoggle{glossarysubstitutionworks}{\glspl{bureaucrat}.}{bureaucrats.}
Also known as a procedure, as in ``policies and procedures.''\\ 
...
description={
Caused by the oversimplification of assumptions and the neglect of specific circumstances. Process friction results in the waste of resources (tangible or expertise), temporal inefficiency, emotional frustration, and social distrust of institutions}
}
