...
\ \\
Multiple interpretations of common sense:
\begin{itemize}
\item Cultural norms
...
Changing a burnt out light bulb at home only takes me a few minutes. I've done that many times.

\marginpar{[Tag] Story Time}
\index{story time!changing a lightbulb}
\begin{mdframed}
In a large organization comprised of specialized roles, an office worker notifies their manager who notifies the building supervisor who submits a request to the maintenance team. Maintenance schedules the repair. An order of 1000 replacement bulbs was made last year and there are some still available in storage. A maintenance team is assigned to the task and deployed. Maintenance team goes to the storage facility to get a ladder and multiple bulbs of multiple models. The team has to sign out the bulbs from storage so inventory can be tracked (theft). A team is needed because solo use of the ladder is prohibited (safety). Multiple bulb models are needed because which model is required is unknown; multiple bulbs decrease the need to go back to storage. Once on site, the maintenance staff finds the bulb needs to be ordered. Maintenance team notifies the supervisor. A new maintenance request is filed by the building manager.
\end{mdframed}
