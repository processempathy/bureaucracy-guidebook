...
def simulate(skill_set_for_tasks:list,
    ...
    while ((tick<max_ticks_to_simulate)):
        ...
        for person_index in range(len(list_of_people)):
            ...
            print("   person",person_index,"has backlog length",len(list_of_people[person_index].backlog_of_tasks))
            ...
            if list_of_people[person_index].status=="idle":
                ...
                if len(list_of_people[person_index].backlog_of_tasks)>0:
                    ...
                    print("      task is",list_of_people[person_index].assigned_task)
                    ...
                    print("   person",person_index,"had no tasks in backlog; got new task from infinite queue")
                    print("      task consists of",list_of_people[person_index].assigned_task)
            ...
            if list_of_people[person_index].status=="coordinating":
                ...
                if list_of_people[person_index].status=="coordinating": 
                    ...
                    if (my_task['skill level'] <= another_person_skill_dict[my_task['specialization']]): # another_person can do task
                        ...
                        list_of_people[person_index].work_journal_per_tick[tick]['outcome'] = "not able to give to person "+str(another_person_id)+" from random search"
                        list_of_people[person_index].work_journal_per_tick[tick]['status is now'] = "coordinating"
